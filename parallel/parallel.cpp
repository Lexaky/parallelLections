...
unsigned round_robin(std::vector <unsigned> v, unsigned n) {
	...
	#pragma omp parallel 
	{
		...
		part_sum[t].value = 0;
		int s = n / T; // Вычисляются количество элементов, которые ТОЧНО обработает каждый поток
		int b = n % T; // Эти элементы остаточные, они распределятся между первыми потоками (их кол-во зависит от b)
